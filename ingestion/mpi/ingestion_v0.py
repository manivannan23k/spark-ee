...
def get_partition_data(input_tiff, input_time_str, sensor_name):
    ...
    temp_file_name = "/mnt/data/temp_data/temp.tif"
    ...
    if src_src_code != 4326:
        ...
        dataset = gdal.Warp(temp_file_name, dataset, format="GTiff", dstSRS="EPSG:4326")
...
if rank == 0:
    ...
    work_data = []
    image_idx = 0
    for idata in ingestion_data:
        start_time = time.time()
        i_work_data = get_partition_data(
            idata["srcPath"], idata["dataTime"], "Landsat_OLI"
        )
        work_data = [*work_data, *i_work_data]
        image_idx += 1
        print(
            f"Image {image_idx}/{len(ingestion_data)} processed",
            "--- %s seconds ---" % (time.time() - start_time),
        )
        # break
    print(f"Total {len(work_data)} tiles")
    send_data = split(work_data, nprocs)
else:
    print("Other node", name, rank)
    send_data = None

partition_datas = comm.scatter(send_data)

print("Do work with the data", len(partition_datas), name, rank)

partition_data(partition_datas)

# mpiexec -machinefile /mnt/data/machinefile -n 34 -x LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH /mnt/data/env/project/bin/python /mnt/data/common/ingestion/partition.py
