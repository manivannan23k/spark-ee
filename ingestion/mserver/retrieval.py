...
def load_data(tile, tindex, sensor_name):
    ...
    tile_data = Db.get_db_tile_by_zindex_zoom_ds(
...
def merge_tiles(merge_ds, projWin):
    ...
    merge_ds = gdal.Translate(
        out_path, merge_ds, format="GTiff", projWin=projWin, projWinSRS="EPSG:4326"
    )
    return out_path
...
def merge_tiles_tmp(merge_ds, projWin):
    ...
    merge_ds = gdal.Warp("", merge_vrt, format="VRT")
    merge_ds = gdal.Translate(
        "", merge_ds, format="VRT", projWin=projWin, projWinSRS="EPSG:4326"
    )
    return merge_ds
