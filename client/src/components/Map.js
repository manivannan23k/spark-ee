    ...
    return <div>
        {
            layers.map(layer => {
                ...
                switch (layer.type) {
                    ...
                    case 'DATA_TILE':
                        return <TileLayer key={layer.id}
                            url={`${Config.DATA_HOST}/tilem/${layer.dsId}/{z}/{x}/{y}.png?tIndexes=${layer.tIndexes.length > 15 ? layer.tIndexes.slice(0, 9).join(",") : layer.tIndexes.join(",")}&bands=${layer.style.bands.join(",")}&vmin=${layer.style.min}&vmax=0.35&aoi_code=${layer.aoiCode}`}
                            maxZoom={24}
                        />
                }
                return '';
