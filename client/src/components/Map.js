...
function ClickEvent(props) {
    ...
    const map = useMapEvent('click', (e) => {
        ...
        fetch(`http://localhost:8080/getPixelAt?sensorName=Landsat8_SC&fromTs=${fromTs}&toTs=${toTs}&y=${e.latlng.lat}&x=${e.latlng.lng}`)
        .then(r=>r.json())
        .then(r=>{
...
const Map = (props) => {
    ...
    useEffect(()=>{
        ...
        dispatch(addLayer({
            ...
            id: 'data',
            active: true,
            url: `http://localhost:8080/tile/Landsat8_SC/{z}/{x}/{y}.png?tIndex=${props.map.tIndex}&bands=${props.map.bands.join(",")}&vmin=0&vmax=${props.map.vizMaxValue}`,
            name: 'Streets - Base',
            sortOrder: 0,
            showLegend: false,
